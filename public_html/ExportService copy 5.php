...
class ExportService
{
    ...
    {
        /* FILA EXCLUIDA → TODO el texto en rojo. Es un indicador visual: el ítem
           salió del cuadro y ya no se va a comprar, así que la fila entera debe
           saltar a la vista al revisar la hoja, no solo la celda de ESTADO.
           El color se inyecta en el estilo base de cada celda; las que fijan su
           propio color (ESTADO_EJEC, y los importes en rojo por sobregiro) lo
           declaran DESPUÉS y por tanto ganan, que es lo correcto. */
        $excluido = self::estadoEjec($r) === 'Excluido';
        $rojoFila = $excluido ? 'color:#C00000;' : '';
        ...
        foreach (self::HEADERS as $key => $_) {
            ...
            $base   = self::NOWRAP . 'background:' . $cellBg . ';' . $rojoFila;
            if (in_array($key, self::INT, true)) {
                ...
                // En una fila excluida TODO va rojo; la celda de estado se
                // distingue con un fondo suave para que siga destacando.
                if ($texto === 'Excluido') $base .= 'background:#FDE8E8;';
                ...
                /* CERT · el check "Cert" del SIGA, igual que en pantalla:
                     ✔ verde  = línea certificada
                     ✘ roja   = certificada pero ANULADA
                     (vacío)  = sin certificación
                   Se usan los caracteres Unicode ✔/✘ en vez de un icono, porque
                   este export es HTML interpretado por Excel y no admite fuentes
                   de iconos. El número de certificación (SIGA · SIAF) va en el
                   title de la celda, que Excel conserva como comentario/tooltip.
                ...
                $out .= '<td title="' . htmlspecialchars($tip) . '" style="' . $base . $col
                      . 'mso-number-format:\'@\';text-align:center;font-weight:bold;font-size:11pt">'
                      . $marca . '</td>';
